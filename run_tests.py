...
class TestU01Arguments:
    ...
    def get_args(self, s0, s1):
        return [self.generator, self.testsuite, self.output, s0, s1, '--output-shift', str(self.output_shift)]
...
class CmdLineArguments:
    ...
    def get_args(self, s0, s1):
        return ['bash', '-c', f'"{self.generator} stdout {self.output} {s0} {s1} | {self.extra_args}"']
...
def run_test(index, testsuite, num_seeds, gen_args, output_directory, dry_run):
    ...
    # Run the job.
    run_command(gen_args.get_args(str(seed_lo64), str(seed_hi64)), working_directory, dry_run)


def run_all_seeds(generator, testsuite, output, num_seeds, output_shift, output_directory, dry_run=False, mppoolsize=DEFAULT_MP_POOL_SIZE):
    ...
    if testsuite.endswith('crush') or \
       testsuite == 'alphabit' or \
       testsuite == 'matrixrank' or \
       testsuite == 'linearcomp':
...
def main():
    ...
    parser.add_argument('testsuite',
                        choices=['smallcrush', 'crush', 'bigcrush', 'alphabit', 'practrand', 'gjrand', 'matrixrank', 'linearcomp'],
    ...
    parser.add_argument('--outputshift',
                        type=int,
                        default=0,
