...
def parse_practrand_report(seed, filepath):
    ...
    if 'no anomalies in' in last_result_text:
        return result
    m = re.search(r'Test Name.*\n((.*\n)+)\s+\.\.\.and (\d+) test result\(s\) without anomalies', last_result_text,
                  re.MULTILINE)
    if not m:
        raise Exception('No results summary')
...
def parse_practrand_report_full(seed, filepath):
    ...
    for line in result_lines:
        ...
        test_p_value = eval(m.group(1))  # Use eval since these numbers can include operators such as '1-7.8e-3'
