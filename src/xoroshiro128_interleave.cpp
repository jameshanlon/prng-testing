...
inline uint64_t plus(uint64_t s0, uint64_t s1) {
  return s0 + s1;
}
...
void set_seed(uint64_t seed0, uint64_t seed1) {
  ...
  // Sanity check: jump forwards 2^128-1. Should be equivalent to jumping forwards by 0.
  ...
  for (generatorIndex = 0; generatorIndex < NUM_GENERATORS; generatorIndex++) {
    ...
    __uint128_t jumpDistance = ((__uint128_t)jumpDistanceHi<<64) | (__uint128_t)jumpDistanceLo;
    //std::cout << "Seed " << generatorIndex << " jump by " << std::hex << jumpDistanceHi << " " << jumpDistanceLo << "\n";
...
#error No seed scheme defined
  ...
  if (s0set.size() != NUM_GENERATORS) {
    std::cerr << "Duplicate s0 seeds (" << std::to_string(NUM_GENERATORS-s0set.size()) << " duplicates)\n";
    for (auto x : s0set) {
      std::cerr << std::to_string(x) << "\n";
    }
  }
  if (s1set.size() != NUM_GENERATORS) {
    std::cerr << "Duplicate s1 seeds (" << std::to_string(NUM_GENERATORS-s1set.size()) << " duplicates)\n";
    for (auto x : s1set) {
      std::cerr << std::to_string(x) << "\n";
    }
  }
