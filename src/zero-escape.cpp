...
int main(int argc, char* argv[]) {
  ...
  // Handle arguments.
  if (argc != 4) {
    std::cerr << "USAGE: " << argv[0] << " <steps> <sample-inerval> <window-size>\n";
    std::cerr << "  Steps:           The number of generator iterations to run for.\n";
    std::cerr << "  Window size:     The number of previous outputs over which to compute an average.\n";
    std::cerr << "  Sample interval: The interval over which to report results (to summarise large nubmers of steps).\n";
    return 1;
  }
