...
std::vector<double> chi2CriticalVals = {0.0,
                                        0.0,
                                        6.251388631170325,
                                        12.017036623780532,
                                        22.307129581578693,
...
uint64_t plus(uint64_t s0, uint64_t s1) { return s0 + s1; }
...
int main(int argc, const char *argv[]) {
  ...
  for (size_t numBits = MIN_NUM_BITS; numBits <= MAX_NUM_BITS; numBits++) {
    std::cout << "Num state bits " << (numBits * 2)
              << " num output bits: " << numBits << "\n";
    ...
    // std::cout << numBits << "bits, spread " << min << " " << max << " " <<
    // (max-min) << "\n";
    ...
    for (size_t i = 0; i < numOutputPatterns; i++) {
      ...
      chiSquared += (double)(countMinusExpected * countMinusExpected) /
