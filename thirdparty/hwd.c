...
#define SIZE                                                                                                     \
  ((DIM >= 1 ? UINT64_C(3) : UINT64_C(1)) * (DIM >= 2 ? 3 : 1) * (DIM >= 3 ? 3 : 1) * (DIM >= 4 ? 3 : 1)         \
   * (DIM >= 5 ? 3 : 1) * (DIM >= 6 ? 3 : 1) * (DIM >= 7 ? 3 : 1) * (DIM >= 8 ? 3 : 1) * (DIM >= 9 ? 3 : 1)      \
   * (DIM >= 10 ? 3 : 1) * (DIM >= 11 ? 3 : 1) * (DIM >= 12 ? 3 : 1) * (DIM >= 13 ? 3 : 1) * (DIM >= 14 ? 3 : 1) \
   * (DIM >= 15 ? 3 : 1) * (DIM >= 16 ? 3 : 1) * (DIM >= 17 ? 3 : 1) * (DIM >= 18 ? 3 : 1) * (DIM >= 19 ? 3 : 1))
...
static void analyze(int64_t pos, bool trans, bool final) {
  ...
  if (pos < 2 * pow(2.0 / (1.0 - P), DIM))
    printf("WARNING: p-values are unreliable, you have to wait (insufficient data for meaningful answer)\n");

  const double pvalue = compute_pvalue(trans);
  const time_t tm     = time(0);

  printf("processed %.3g bytes in %.3g seconds (%.4g GB/s, %.4g TB/h). %s\n", (double)pos, (double)(tm - tstart),
         pos * 1E-9 / (double)(tm - tstart), pos * (3600 * 1E-12) / (double)(tm - tstart), ctime(&tm));
...
static int64_t progsize[] = {100000000, 125000000, 150000000, 175000000, 200000000, 250000000, 300000000,
...
int main(int argc, char** argv) {
  ...
  cs = mmap((void*)(0x0UL), (SIZE + 1) * sizeof *cs + SIZE * sizeof *norm + SIZE * sizeof *count_sum,
            PROT_READ | PROT_WRITE, MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB | (30 << MAP_HUGE_SHIFT), 0, 0);
  if (cs == MAP_FAILED) {
