...
main() {
  ...
  for (gen_type = 0; gen_type < 2; gen_type++) {
    ...
    if (gen_type == 0) {
      ...
      y = 16;
      z = 37;
      printf("NB: NOT Graphcore's xoroxhiro LFSR. Shift distances %d,%d,%d. "
             "For comparison with code in "
    ...
    for (i = 64; i <= 96; i = i + 32) {
      ...
      printf("jump poly for xoroshiro-%d-%d-%d for dist 2^%3d = 0x", x, y, z,
             i);
      j = printvechc(cpolyj);
    }
    ...
    printf("jump poly for xoroshiro-%d-%d-%d for d=(2^128)-1 == zero (sanity "
           "check, jump polynomial should=1) 0x",
           x, y, z);
    j = printvech(cpolyj);
    printf("\n");
  }
